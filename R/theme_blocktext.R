theme_blocktext <- function(df_prices, theme_input){
  ...
  sentence_part_2 <- case_when(
    ...
    theme_end == current_year ~ glue::glue("est toujours actif en {theme_end} avec {theme_end - theme_launch} ans d'existence."),
    theme_end != current_year ~ glue::glue("a été arrêté en {theme_end}, après {theme_end - theme_launch} ans d'existence.")
  )
